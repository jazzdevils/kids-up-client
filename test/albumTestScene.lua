...
local function listButtonEvent( event )
    if event.phase == "ended" then
        ...
        api.get_album_list("7", "1", "10", callbackListener)
    end
...
local function list2ButtonEvent( event )
    if event.phase == "ended" then
        native.setActivityIndicator( true )
        api.get_album_list2("71", "119", "1", "10", callbackListener)
    end
...
local function addButtonEvent( event )
    if event.phase == "ended" then
        ...
        local params = {
            ...
            thread_type = "6",
            thread_id = "2",
            filename = "acba15156caf2217caea57b10f4e9bbe3026e381.jpg"
        }
        api.add_album_data(params, callbackListener)
    end
...
local function deleteButtonEvent( event )
    if event.phase == "ended" then
        native.setActivityIndicator( true )
        api.delete_album_data("7", "3", "1", callbackListener)
    end
...
function scene:enterScene( event )
    ...
    list2.x = display.contentCenterX
    list2.y = list.y + 50
    group:insert(list2)    
