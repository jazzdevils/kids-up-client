...
local function changeClassButtonEvent( event )
    if event.phase == "ended" then
        native.setActivityIndicator( true )
        api.change_teacher_class("70", "191", callbackListener)
    end
    
    return true
end

local function voidApprovalButtonEvent( event )
    if event.phase == "ended" then
        native.setActivityIndicator( true )
        api.void_teacher_approval("70", callbackListener)
    end
...
local function listButtonEvent( event )
    if event.phase == "ended" then
        native.setActivityIndicator( true )
        api.get_teacher_list("16", callbackListener)
    end
...
function scene:enterScene( event )
    ...
    local changeClass = widget.newButton
    {
        width = 130 ,
        height = 40 ,
        ...
        fontSize = __buttonFontSize__,
        label = "change class",
        onRelease = changeClassButtonEvent
    }
    changeClass.x = display.contentCenterX
    changeClass.y = navBar.height + 30
    group:insert(changeClass)    

    local voidApproval = widget.newButton
    {
        width = 130 ,
        height = 40 ,
        defaultFile = "images/button_inframe/btn_inframe_blue_3_normal.png",
        ...
        fontSize = __buttonFontSize__,
        label = "void approval",
        onRelease = voidApprovalButtonEvent
    }
    voidApproval.x = display.contentCenterX
    voidApproval.y = changeClass.y + 50
    group:insert(voidApproval)
