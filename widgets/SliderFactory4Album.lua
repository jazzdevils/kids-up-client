...
function Factory.newScrollView(_imgPath, _W, _H)
    ...
    function group:setImage(imgPath, imgInfo)
        if (imgPath == nil) then return end

        local function viewInfo()
            if infoGroup then 
            ...
            local memo_width = _W - 20
--            local memo_height = rect.height - 10
            local sMemo = imgInfo.memo
            if(utils.UFT8Len(sMemo) > __CONTENTS_LIMIT_LENGTH__) then
                sMemo = utils.UTF8Sub(sMemo, 1, __CONTENTS_LIMIT_LENGTH__) .. "..."
            end
            
            local tmp_memo_txt = display.newText(sMemo, 10, __statusBarHeight__ + 10, memo_width, 0, native.systemFont, __textLabelFont12Size__)
            local tmp_memo_height = tmp_memo_txt.height
            tmp_memo_txt:removeSelf()
            tmp_memo_txt = nil
            
            local rect = display.newRect(0, 0, _W, _H)
            rect.anchorY = 0
            rect.height = 75 + tmp_memo_height
            rect.width = _W
            rect.x = display.contentCenterX
            rect.y = _H - rect.height
            rect:setFillColor(1, 1, 1, 0.6)
            infoGroup:insert(rect)
            
            local center_name = display.newText(imgInfo.center_name, 10, __statusBarHeight__ + 10, native.systemFont, 12)
            center_name.anchorX = 0
            center_name.anchorY = 0
            center_name.x = 10
            center_name.y = rect.y + 10
            center_name:setFillColor(0,0,0)
            infoGroup:insert(center_name)
            
            local memo_txt = display.newText(sMemo, 10, __statusBarHeight__ + 10, memo_width, 0, native.systemFont, __textLabelFont12Size__)
            memo_txt.anchorX = 0
            memo_txt.anchorY = 0
            memo_txt.x = center_name.x 
            memo_txt.y = center_name.y + center_name.height + 5
            memo_txt:setFillColor(0,0,0)
            infoGroup:insert(memo_txt)
            
            local dateText = utils.convert2LocaleDateStringFromYYYYMMDD(imgInfo.createtime)
            local date_txt = display.newText(dateText, 10, __statusBarHeight__ + 10, native.systemFont, 10)
            date_txt.anchorX = 0
            date_txt.anchorY = 0
            date_txt.x = memo_txt.x
            date_txt.y = memo_txt.y + memo_txt.height + 5
            date_txt:setFillColor(0,0,0)
            infoGroup:insert(date_txt)
            
            infoGroup.alpha = 1
            
            group:insert(infoGroup)
        end
            
        if(utils.fileExist(imgPath, path) == true) then
            if (img) then 
                img:removeSelf()
            end
            ...
            img.anchorX = 0
            img.anchorY = 0
            img.y = (_H - img.height)/2 
            img.isLoadingComplete = true
    ...
    function group:touch( event )
            ...
            elseif img.isFocus then
                    ...
                    elseif "ended" == phase or "cancelled" == phase then
                            ...
                            if(infoGroup.alpha == 1) then
                                transition.to(infoGroup, {alpha = 0})
                            else
                                transition.to(infoGroup, {alpha = 1})
                            end
...
function Factory.newImageSlider(_W, _H)
    ...
    local imagesInfo -- collection
    
    setCenterScrollView = function (_center, _left, _right)
        if images[isv.selectedIndex - 1] then 
            _left:setImage(images[isv.selectedIndex - 1], imagesInfo[isv.selectedIndex - 1]) 
        end
        if images[isv.selectedIndex] then 
            _center:setImage(images[isv.selectedIndex], imagesInfo[isv.selectedIndex]) 
        end
        if images[isv.selectedIndex + 1] then 
            _right:setImage(images[isv.selectedIndex + 1], imagesInfo[isv.selectedIndex + 1]) 
        end
    ...
    function isv:setDataProvider(_images, _path, idx, _imageInfo) -- idx = firstPage
        images = _images
        path = _path
        imagesInfo = _imageInfo
