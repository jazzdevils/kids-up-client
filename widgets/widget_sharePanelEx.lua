...
function widget.newSharingPanelEx()
    ...
    local function tableViewListener(event)
        ...
        elseif phase == "ended" then
            ...
            local imageMaxCount = row.params.parent.imageMaxCount
            ...
            if (service == "takephoto") then
                ...
                timer.performWithDelay( 500, 
                    function() 
                        storyboard.purgeScene("scripts.takePhotoSceneEx")
                        storyboard.gotoScene("scripts.takePhotoSceneEx", options) 
                    end 
                )
            elseif (service == "getphoto") then
--                local alert = native.showAlert( "subMenu", "get Photo", { "OK" }, nil)
                ...
                timer.performWithDelay( 500, 
                    function() 
                        storyboard.purgeScene("scripts.getPhotoSceneEx")
                        storyboard.gotoScene("scripts.getPhotoSceneEx", options) 
                    end 
                )
...
--                local imageMaxCount = event.row.params.parent.imageMaxCount
--                
--                if (service == "takephoto") then
...
--        onRowTouch = onRowTouch,
    ...
    tableView:insertRow{
        ...
        params = {
            parent = panel,
            service = "takephoto",
            label = language["widget_sharePanelEx"]["take_photo"],
    ...
    tableView:insertRow{
        ...
        params = {
            parent = panel,
            service = "getphoto",
            label = language["widget_sharePanelEx"]["get_photo"],
    ...
    tableView:insertRow{
        ...
        params = {
            parent = panel,
            service = "cancel",
            label = language["widget_sharePanelEx"]["cancel"],
