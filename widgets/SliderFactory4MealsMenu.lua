...
function Factory.newScrollView(_imgPath, _W, _H)
    ...
    function group:setImage(imgPath, imgInfo)
        ...
        local function viewInfo()
            ...
            local tmp_memo_txt = display.newText(imgInfo.title, 10, __statusBarHeight__ + 10, _W - 20, 0, native.systemFont, __textLabelFont12Size__)
            ...
            rect.anchorY = 0
            rect.height = 60 + tmp_memo_height
            rect.width = _W
            ...
            local sYear = string.sub(selected_menu_date, 1, 4)
            local sMonth = string.sub(selected_menu_date, 5, 6)
            local sDay = string.sub(selected_menu_date, 7, 8)
            local strTmp = utils.convert2LocaleDateString(sYear, sMonth, sDay).. " "..language["mealMenuListScene"]["menu"]
            local menu_date = display.newText(strTmp, 10, __statusBarHeight__ + 10, native.systemFont, __textLabelFont12Size__)
            menu_date.anchorX = 0
            menu_date.anchorY = 0
            menu_date.x = 10
            menu_date.y = rect.y + 10
            menu_date:setFillColor(0,0,0)
            infoGroup:insert(menu_date)
            
            local memo_width = _W - 20
            local memo_height = rect.height - 10
            local memo_txt = display.newText(imgInfo.title, 10, __statusBarHeight__ + 10, memo_width, memo_height, native.systemFont, __textLabelFont12Size__)
            memo_txt.anchorX = 0
            memo_txt.anchorY = 0
            memo_txt.x = menu_date.x 
            memo_txt.y = menu_date.y + menu_date.height + 5
...
function Factory.newImageSlider(_W, _H)
    ...
    function isv:setDataProvider(_images, _path, idx, _imageInfo, _selected_menu_date) -- idx = firstPage
        images = _images
        path = _path
        imagesInfo = _imageInfo
        selected_menu_date = _selected_menu_date
