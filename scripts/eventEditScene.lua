...
local function onRightButton(event)
    if event.phase == "ended" then
        ...
        if(title_textBox and title_textBox.text ~= "") then
            if(content_textBox and content_textBox.text ~= "") then
                local params = {}
                params.event_id = eventParams.event_id
                params.class_id = eventParams.classid
                params.title = title_textBox.text
                params.contents = content_textBox.text
                params.address = address_textBox.text
                
                local eventDate = eventParams.eventDate
                local strEventDate = ""
                ...
                api.update_event_contents(params, getDataCallback)
            else
                utils.showMessage(language["eventEditScene"]["input_contents"])
                native.setKeyboardFocus(content_textBox) 
                
                return true
            end
        else
            utils.showMessage(language["eventEditScene"]["input_title"])
...
local function viewClass()
    ...
    classes[1] = {id = "",name = language["eventEditScene"]["all_class"], desc = ""}
    class_label[1] = language["eventEditScene"]["all_class"]
    for i = 1, class_cnt do
        ...
        if(class.id == eventParams.classid) then
            startIndexClass = i+1
        end
    end
...
--            pickerHeight = 130,
            ...
            titleText = language["eventEditScene"]["select_class"],
--            onScroll = nil,
            okButtonText = language["eventEditScene"]["ok"],
            onClose =   function()
                            native.setKeyboardFocus(title_textBox)
            ...
            onOKClick = function(event)
                            if(event.phase == "ended") then
                                ...
                                eventParams.classid = classData.id
...
local function onAddDateButtonClick(event)
    ...
    local selectedMonth
    local selectedDay
    if ( eventParams.eventDate ) then
        selectedYear = eventParams.eventDate.year
        selectedMonth = eventParams.eventDate.month
        selectedDay = eventParams.eventDate.day
    end
    ...
    for d = 1, 31 do
        days[d] = d..language["eventEditScene"]["day"]
    end
    ...
    for y = 1, 3 do
        years[y] = (startYear + y) ..language["eventEditScene"]["year"]
    end
    ...
    local function onScroll(event)
        ...
        local a, b = string.find(values[3].value, language["eventEditScene"]["day"])
    ...
    columnData[1].startIndex =  selectedYear - currentYear + 1
...
--            editField = row.pickerField,
            pickerData = columnData,
            titleText = language["eventEditScene"]["input_date"],
            onScroll = onScroll,
            okButtonText = language["eventEditScene"]["ok"],
            onClose =   function()
--                            native.setKeyboardFocus(title_textBox)
...
--    end    
    ...
    eventParams = sceneData.getSceneDataWithUID("eventViewScene")
    ...
    local btn_left_opt = {
        ...
        label = language["eventEditScene"]["cancel"],
    ...
    local btn_right_opt = {
        labelColor = { default = __NAVBAR_BUTTON_COLOR__, over = __NAVBAR_BUTTON_COLOR__ },
        label = language["eventEditScene"]["ok"],
    ...
    local navBar = widget.newNavigationBar({
        title = language["eventEditScene"]["title"],
...
--        includeStatusBar = true
    ...
    local selectClassText = display.newText(language["eventEditScene"]["short_select_class"], 0, 0, native.systemFont, 12)
    ...
    local class_name = ""
    if(eventParams.classid and eventParams.classid ~= "") then
        class_name = user.getClassName(eventParams.classid)
    else
        class_name = language["eventEditScene"]["all_class"]
    end
    ...
    if (authority.validAuthorityByUser(__SELECT_CLASS__) == true) then
...
function scene:enterScene( event )
    ...
    storyboard.returnTo = "scripts.eventViewScene"
        
    local function title_inputListener( event )
        if event.phase == "began" then
            if (group.y < 0) then
                transition.to(group, {y = 0, time=100, onComplete=nil})
            end
    ...
    local function contents_inputListener( event )
        if event.phase == "began" then
            transition.to(group, {y = - contentField_Move_Y, time=100, onComplete=nil})
            if(button_keyboardOnOff) then
    ...
    local function address_inputListener( event )
        if event.phase == "began" then
            transition.to(group, {y = - addressField_Move_Y, time=100, onComplete=nil})
...
--            native.setKeyboardFocus(content_textBox)
    ...
    title_textBox.text = ""        
    title_textBox.placeholder = language["eventEditScene"]["input_title"]
    ...
    local title_text = eventParams.eventTitle
    if(title_text) then
    ...
    content_textBox.placeholder = language["eventEditScene"]["input_contents"]
    ...
    local content_text = eventParams.eventContents
    ...
    address_textBox.placeholder = language["eventEditScene"]["map_desc"]
    address_textBox.hasBackground = false
    address_textBox.font = native.newFont(native.systemFont, 10)
    address_textBox:addEventListener("userInput", address_inputListener)
    local address_text = eventParams.mapAddress
        ...
        onRelease = function(event)
                        if(event.phase == "ended") then
                            if(address_textBox.text ~= "") then
                                eventParams.eventTitle = title_textBox.text
                                eventParams.eventContents = content_textBox.text
                                eventParams.mapAddress = address_textBox.text
                                sceneData.addSceneDataWithUID("mapAddress", eventParams.mapAddress)
                                ...
                                utils.showMessage(language["eventEditScene"]["input_address"])
...
--    eventDateRect:setFillColor(1, 0, 0, 1)
    ...
    local eventDateText = display.newText(language["eventEditScene"]["short_input_date"], 0, 0, native.systemFont, 12)
...
--                end
    ...
    local eventDate_data = eventParams.eventDate
    local eventDate_txt
