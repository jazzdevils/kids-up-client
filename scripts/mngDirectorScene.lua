...
local function onRowTouch(event)
    ...
    if(event.phase == "release") then
        ...
        if(index == 1)then
            ...
            storyboard.gotoScene("scripts.mngInviteScene", options)     -- 초대코드
        elseif(index == 3)then
            storyboard.isAction = true
            storyboard.purgeScene("scripts.askApprovalScene")
            storyboard.gotoScene("scripts.askApprovalScene", options)   -- 미승인리스트
        elseif(index == 4)then
            storyboard.isAction = true
            storyboard.purgeScene("scripts.mngTeacherListScene")
            storyboard.gotoScene("scripts.mngTeacherListScene", options)-- 선생 관리
        elseif(index == 2)then    
            storyboard.isAction = true
            storyboard.purgeScene("scripts.mngClassListScene")
            storyboard.gotoScene("scripts.mngClassListScene", options)  -- 반 관리
        elseif(index == 5)then
            storyboard.isAction = true
            storyboard.purgeScene("scripts.mngClassKidsScene")
            storyboard.gotoScene("scripts.mngClassKidsScene", options)  -- 아이 관리
        elseif(index == 6)then
            storyboard.isAction = true
            storyboard.purgeScene("scripts.mngRollBookScene")
            storyboard.gotoScene("scripts.mngRollBookScene", options) --출석부
--        elseif(index == 7)then 
--            storyboard.isAction = true
--            storyboard.purgeScene("scripts.mngMsgGetTimeScene") --푸시 시간 설정
...
local function onRowRender(event)
    ...
    if index == 1 then -- 초대코드
        ...
        row.invite_txt = display.newText(language["mngDirectorScene"]["invite_code"],0, 0, native.systemFontBold, 12)
    ...
    elseif index == 3 then
        ...
        row.unapproval_icon = display.newImageRect("images/assets1/icon_setting_cnf_teacher.png", 24 , 24)
        ...
        row.unapproval_txt = display.newText(language["mngDirectorScene"]["notapplist"],0, 0, native.systemFontBold, 12)
    ...
    elseif index == 4 then
        ...
        row.manageteacher_icon = display.newImageRect("images/assets1/icon_setting_mng_teacher.png", 24 , 24)
        row.manageteacher_icon.anchorX = 0
        row.manageteacher_icon.anchorY = 0
        row.manageteacher_icon.x = 10
        row.manageteacher_icon.y = (row.height - row.manageteacher_icon.height) /2
        row:insert(row.manageteacher_icon)
        
        row.manageteacher_txt = display.newText(language["mngDirectorScene"]["manage_teacher"],0, 0, native.systemFontBold, 12)
        row.manageteacher_txt.anchorX = 0
        row.manageteacher_txt.anchorY = 0
        row.manageteacher_txt:setFillColor(0, 0, 0)
        row.manageteacher_txt.x = row.manageteacher_icon.x + row.manageteacher_icon.width + 10
        row.manageteacher_txt.y = (row.height - row.manageteacher_txt.height) /2
        row:insert(row.manageteacher_txt)
    elseif index == 2 then
        row.rect = display.newRoundedRect(row.width/2, row.height/2, row.width - 10, row.height - 6, 6)
        row:insert(row.rect )
        
        row.manageclass_icon = display.newImageRect("images/assets1/icon_setting_mng_class.png", 24 , 24)
        row.manageclass_icon.anchorX = 0
        row.manageclass_icon.anchorY = 0
        row.manageclass_icon.x = 10
        row.manageclass_icon.y = (row.height - row.manageclass_icon.height) /2
        row:insert(row.manageclass_icon)
        
        row.manageclass_txt = display.newText(language["mngDirectorScene"]["manage_class"],0, 0, native.systemFontBold, 12)
        row.manageclass_txt.anchorX = 0
        row.manageclass_txt.anchorY = 0
        row.manageclass_txt:setFillColor(0, 0, 0)
        row.manageclass_txt.x = row.manageclass_icon.x + row.manageclass_icon.width + 10
        row.manageclass_txt.y = (row.height - row.manageclass_txt.height) /2
        row:insert(row.manageclass_txt)
        
        flickering_txt = row.manageclass_txt
    elseif index == 5 then
        row.rect = display.newRoundedRect(row.width/2, row.height/2, row.width - 10, row.height - 6, 6)
        row:insert(row.rect )
        ...
        row.managekids_txt = display.newText(language["mngDirectorScene"]["managekids"],0, 0, native.systemFontBold, 12)
...
--            size = 16,
    ...
    navBar = widget.newNavigationBar({
            title = language["mngDirectorScene"]["title"],
    ...
    if utils.IS_Demo_mode( storyboard, false) == false then
        if user.userData.jobType == __DIRECTOR__ and user.userData.approvalState == '1' then 
            if #user.classList == 0 then --원장이면서 인증을 받았으나 아직 반 개설이 안되어 있는경우 메세지...
                if flickering_txt then
                    local function onPageSwipeOnTimer( event )
                        if flickering_txt.alpha == 0 then
                           transition.to(flickering_txt, {time = 400, alpha = 1})
                        else
                           transition.to(flickering_txt, {time = 400, alpha = 0})
                        end
                    end
                ...
                if flickering_txt and flickering_txt.alpha ~= 1 then
                    flickering_txt.alpha = 1
                end
            end
        end
    end
