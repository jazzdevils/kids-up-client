...
local function touchStateListener( event )
    ...
    centerAddressRefData.stateId = img.stateId
    centerAddressRefData.stateName = img.stateName
...
local function onRowRender( event )
    ...
    local unitWidth = popupWidth / 3
    local unitRG = display.newGroup()
    if #states > 0 then
        for i = 1, 3 do
            local stateIdx = (row.id - 1) * 3 + i
            if states[stateIdx] then
                local gx = (i - 1) * unitWidth + unitWidth / 2
                local rowBgImg
                if row.index % 2 == 1 then
                    rowBgImg = display.newImageRect(row, "images/assets2/bg_popup_table_odd_3.png", unitWidth - 4, rowHeight - 4)
                else
                    rowBgImg = display.newImageRect(row, "images/assets2/bg_popup_table_even_3.png", unitWidth - 4, rowHeight - 4)
                end
                rowBgImg.x = gx
                rowBgImg.y = rowHeight * 0.5
                local rowTitle = display.newText( unitRG, states[stateIdx].name, 0, 0, nil, 14 )
                rowTitle:setFillColor( unpack(__POPUP_TABLE_ROW_TXT_COLOR__) )
                rowTitle.x = gx
                rowTitle.y = rowHeight * 0.5
                rowBgImg.stateId = states[stateIdx].id
                rowBgImg.stateName = states[stateIdx].name
                ...
                rowTitle.x = gx
                rowTitle.y = rowHeight * 0.5
                rowBgImg.stateId = nil
                rowBgImg.stateName = nil
...
local function renderTableView( data )
    ...
    {
        parent = localGroup,
        text = language["joinScene"]["state_select_txt"],
        x = popupTitleRec.x,
        y = popupTitleRec.y,
        width = popupWidth,
    ...
    if data.state_cnt > 0 then
        local forCnt = 0
        if ( data.state_cnt % 3 == 0) then
            forCnt = data.state_cnt / 3
        else
            forCnt = data.state_cnt / 3 + 1
        end
        for i = 1, forCnt do
            tableView:insertRow(
                {
...
function scene:createScene( event )
    ...
    api.state_list_api(centerAddressRefData.countryId, centerAddressRefData.centerType, centerAddressRefData.cntFlag, listCallback)
end
