...
local function onAddScheduleButtonClick(event)
    ...
    for d = 1, 31 do
        days[d] = d..language["noticeViewScene"]["day"]
    end
    ...
    for y = 1, 3 do
        years[y] = (startYear + y) ..language["noticeViewScene"]["year"]
    end
    ...
    local function onScroll(event)
        ...
        local a, b = string.find(values[3].value, language["noticeViewScene"]["day"])
...
--            editField = row.pickerField,
            ...
            titleText = language["noticeViewScene"]["input_date"],
            onScroll = onScroll,
            okButtonText = language["noticeViewScene"]["ok"],
            ...
            onOKClick = function(event)
                            if(event.phase == "ended") then
                                if (utils.isValidDate(selectedYear, selectedMonth, selectedDay) == true) then
                                    ...
                                    api.add_schedule_data_thread2(user.userData.centerid, user.userData.id, user.getActiveKid_IDByAuthority()
                                         , __NOTICE_THREAD_TYPE__, notice_id, strYear..strMonth..strDay, ""
                                         ,  function(e)
                                                ...
                                                if ( e.isError ) then
                                                    ...
                                                    if(e.status == 200) then
                                                        utils.showMessage(language["ViewSceneButton"]["schedule_save"])
...
local function displayView()
    if(noticeTable) and (oData) then
        local imagesHeight = getHeightofImages(oData)
        local contentRowHeight = getHeightofString(oData.contents, noticeTable.width - 22, native.systemFont, __VIEW_SCENE_TEXT_SIZE__)        
    ...
    if(rowData) and (index == 2)  then
        ...
        if(rowData.class.id ~= "") then
            if(rowData.type == "1") then
                --전체공지
                className =  "[ "..language["noticeViewScene"]["all_class"].." ]"
            elseif(rowData.type == "2") then
                --개별공지
                className =  "[ "..rowData.class.name .." ".. language["noticeViewScene"]["notice"].." ]"
            end
        else
            --원장이 보낸글
            className =  "[ "..language["noticeViewScene"]["all_class"].." ]"
        ...
        row.txt_title.y = row.txt_className.y + row.txt_className.height + 3
        ...
        row.txt_createTime.y = row.txt_title.y + row.txt_title.height + 3
        ...
        if(userSetting.settings.toTranslatorLanguageCode ~= "")then
            row.transButton = display.newText(language["noticeViewScene"]["translation"], 0, 0, native.systemFont, 12)
...
--            emboss = true,
            ...
            onRelease = function(event)
                            if(event.phase == "ended") then
                                ...
                                activityIndicator = ActivityIndicator:new(language["activityIndicator"]["save"])
                                api.plus_notice_goodcnt(notice_id, user.userData.id, 
                                    function() 
                                        activityIndicator:destroy()
...
--            emboss = true,
        ...
        row.okButton = widget.newButton
        {
            ...
            label = language["noticeViewScene"]["ok"],
            labelAlign = "right",
            labelColor = { default={ 1, 1, 1 }, over={ 0, 0, 0, 0.5 } },
--            emboss = true,
            fontSize = __buttonFontSize__,
            onRelease = function(event)
                            if(event.phase == "ended") then
                                ...
                                activityIndicator = ActivityIndicator:new(language["activityIndicator"]["save"])
                                api.send_notice_read(notice_id, user.userData.id, 
                                    function(e)
                                        if (activityIndicator) then
                                            activityIndicator:destroy()
                                        end
        ...
        row.notReadListButton = widget.newButton --공지안읽은 사람 확인 용(원장, 선생만 가능)
        {
            width = row.addScheduleButton.width,
            height = 30,
            defaultFile = "images/assets1/btn_ok_big.png",
            ...
            onRelease = function(event)
                            if(event.phase == "ended") then
                                if(rowData.target.read_cnt == rowData.target.total_cnt) then
                                    utils.showMessage(language["noticeViewScene"]["allconfirm"])
                                else
                                    local options = {
                                        effect = "fromRight",
                                        time = 300,
                                        params = {
                                            thread_type = __NOTICE_THREAD_TYPE__,
                                            thread_id = notice_id,
                                            ...
                                            previous_scene = "scripts.noticeViewScene"
                                        }
                                    }
                                    storyboard.purgeScene("scripts.notReadListScene")
            ...
            row.deleteButton = widget.newButton
            {
                ...
                onRelease = function(event)
                                if(event.phase == "ended") then
                                    ...
                                    native.showAlert(language["appTitle"], language["noticeViewScene"]["delete_comment"], 
                                        { language["noticeViewScene"]["yes"], language["noticeViewScene"]["no"] }, 
                                        function(event)
                                            if "clicked" == event.action then
                                                local i = event.index
                                                if 1 == i then
                                                    activityIndicator = ActivityIndicator:new(language["activityIndicator"]["delete"])
                                                    api.delete_notice_reply(rowData.notice_id, rowData.reply_id, 
                                                        function(e)
                                                            oData.needRefresh = true
                                                            activityIndicator:destroy()
                                                            
                                                            noticeTable._view._velocity = 0
                                                            noticeTable:deleteRow(index)
                                                        end
                                                    )
                                                end
                                            end    
...
local function scrollListener( event )
	...
	reloadspinner.alpha = 1
        ...
        pageno = 1
        api.notice_reply_get_api2(notice_id, pageno, pagesize, getCommentDataCallback)
    ...
    if event.phase == "ended" then
        if(user.userData.jobType ~= __DIRECTOR__)then
            if(oData.writer.id ~= user.userData.id) then
                utils.showMessage(language["noticeViewScene"]["authority_edit"])
                return true
            end
        end
        ...
        sceneData.addSceneDataWithUID("noticeTitle", oData.title)
        sceneData.addSceneDataWithUID("noticeContents", oData.contents)
        ...
        local options =
        {
            ...
            {
                notice_id = notice_id,
                notice_toWhere = oData.type,
                notice_classid = oData.class.id
            }
        }
        storyboard.purgeScene("scripts.noticeEditScene")
        storyboard.gotoScene("scripts.noticeEditScene", options)
    end
...
local function onRightSideButton(event)
    if event.phase == "ended" then --공지사항 삭제
        if utils.IS_Demo_mode(storyboard, true) == true then
            return true
        end
        ...
        if(user.userData.jobType ~= __DIRECTOR__)then
            if(oData.writer.id ~= user.userData.id) then
                utils.showMessage(language["noticeViewScene"]["authority_delete"])
                return true
            end
        end
        
        native.showAlert(language["appTitle"], language["noticeViewScene"]["delete_question"], 
            { language["noticeViewScene"]["yes"],language["noticeViewScene"]["no"] },  
            function(event) 
                if "clicked" == event.action then
                    local i = event.index
...
--    rowData = event.params.rowData
    oData = sceneData.getSceneData("scripts.noticeScene", "scripts.noticeViewScene")
    notice_id = oData.id
...
--	onRowTouch = nil,--onRowTouch,
    ...
    noticeTable.x = display.contentWidth / 2
    table_group:insert(noticeTable)
    group:insert(table_group)
    
    local btn_left_opt = {
        labelColor = { default = __NAVBAR_BUTTON_COLOR__, over = __NAVBAR_BUTTON_COLOR__},
        label = language["noticeViewScene"]["back"],
...
--        label = "削除",
    ...
    if(authority.validAuthorityByUser(__NOTICE_WRITE__) == true) then    
        navBar = widget.newNavigationBar({
            title = language["noticeViewScene"]["title"],
            width = __appContentWidth__,
...
--    comment_box.anchorX = 0
    ...
    local default_comment_text = display.newText(group, language["noticeViewScene"]["input_comment"], 0, 0, __appContentWidth__ - 50, 30, native.systemFont, 12)
