...
local function onRowRender( event )
    ...
    if(rowData) and (index == 2)  then
        ...
        local sTitle =  "[ "..rowData.class.name .." ]".."  "..rowData.title
        if string.len(sTitle) > 70 then
            sTitle = string.sub(sTitle, 1, 70) .. "..."
        end
        ...
        row.txt_createTime.x = row.txt_title.x
--        row.txt_createTime.y = TOP_PADDING + row.txt_title.height + row.underline.height
        row.txt_createTime.y = TOP_PADDING + row.txt_title.height
        ...
        if(userSetting.settings.toTranslatorLanguageCode ~= "")then
            row.transButton = display.newText(language["mamatalkViewScene"]["translation"], 0, 0, native.systemFont, 12)
...
--            sumY = sumY + rowData.img9.new_h + contentImagePadding
        ...
        row.goodButton = widget.newButton
        {
            ...
            label = language["mamatalkViewScene"]["good"],
            labelAlign = "right",
...
--            emboss = true,
            ...
            onRelease = function(event)
                            if(event.phase == "ended") then
                                ...
                                api.plus_mamatalk_goodcnt(mamatalk_id, user.userData.id, 
                                    function() 
...
--            onRelease = function(event)
--                            if(event.phase == "ended") then
--                                api.send_notice_read(mamatalk_id, user.userData.id, function() end)
--                            end
--                        end
--        }
...
--        
--        row.readListButton = widget.newButton --공지읽은 사람 확인 용(원장, 선생만 가능)
--        {
--            width = row.addScheduleButton.width,
--            height = 30,
--            defaultFile = "images/assets1/btn_ok_big.png",
...
--                        end
--        }
--        row.readListButton.anchorX = 0
--        row.readListButton.anchorY = 0
--        row.readListButton.x = cell_width + row.addScheduleButton.x
--        row.readListButton.y = row.addScheduleButton.y
--        row:insert(row.readListButton)
        
--        if(user.userData.jobType == __PARENT__) then
--            row.goodButton.isVisible = true
--            row.addScheduleButton.isVisible = true
--            
            ...
            row.deleteButton = widget.newButton
            {
                ...
                onRelease = function(event)
                                if(event.phase == "ended") then
                                    ...
                                    native.showAlert(language["appTitle"], language["mamatalkViewScene"]["delete_comment"], 
                                        {language["mamatalkViewScene"]["yes"], language["mamatalkViewScene"]["no"] }, 
                                        function(event)
                                            if "clicked" == event.action then
                                                local i = event.index
                                                if 1 == i then
                                                    activityIndicator = ActivityIndicator:new(language["activityIndicator"]["delete"])
                                                    api.delete_mamatalk_reply(rowData.mamatalk_id, rowData.reply_id, 
                                                        function(e)
                                                            oData.needRefresh = true
                                                            activityIndicator:destroy()
                                                            
                                                            mamatalkTable._view._velocity = 0
                                                            mamatalkTable:deleteRow(index)
                                                        end
                                                    )
                                                end
                                            end    
...
local function displayView()
    if(mamatalkTable) and (oData) then
        local imagesHeight = getHeightofImages(oData)
        local contentRowHeight = getHeightofString(oData.contents, mamatalkTable.width - 22, native.systemFont, __VIEW_SCENE_TEXT_SIZE__) 
...
local function scrollListener( event )
	...
	reloadspinner.alpha = 1
        ...
        pageno = 1
        api.get_mamatalk_reply_list2(mamatalk_id, pageno, pagesize, getCommentDataCallback)
...
local function onRightButton(event)
    if event.phase == "ended" then
        if(oData.writer.id ~= user.userData.id) then
            utils.showMessage(language["mamatalkViewScene"]["authority_edit"])
            return true
        end
        ...
        sceneData.addSceneDataWithUID("mamatalkTitle", oData.title)
        sceneData.addSceneDataWithUID("mamatalkContents", oData.contents)
        ...
        storyboard.purgeScene("scripts.mamatalkEditScene")
        storyboard.gotoScene("scripts.mamatalkEditScene", options)
    end
...
local function onRightSideButton(event)
    if event.phase == "ended" then --삭제
        if(oData.writer.id ~= user.userData.id) then
            utils.showMessage(language["mamatalkViewScene"]["authority_delete"])
            return true
        end
        
        native.showAlert(language["appTitle"], language["mamatalkViewScene"]["delete_question"], 
            { language["mamatalkViewScene"]["yes"],language["mamatalkViewScene"]["no"] },  
            function(event) 
                if "clicked" == event.action then
                    local i = event.index
                    if 1 == i then
                        activityIndicator = ActivityIndicator:new(language["activityIndicator"]["delete"])
                        api.delete_mamatalk(mamatalk_id, 
                            function(event) 
                                activityIndicator:destroy()
                                if(event.isError) then
...
function scene:createScene( event )
    ...
    oData = sceneData.getSceneData("scripts.mamatalkScene", "scripts.mamatalkViewScene")
    mamatalk_id = oData.id
...
--	onRowTouch = nil,--onRowTouch,
    ...
    mamatalkTable.x = display.contentWidth * 0.5
    table_group:insert(mamatalkTable)
    group:insert(table_group)
    
    local btn_left_opt = {
        labelColor = { default = __NAVBAR_BUTTON_COLOR__, over = __NAVBAR_BUTTON_COLOR__},
        label = language["mamatalkViewScene"]["back"],
...
--        label = "削除",
    ...
    navBar = widget.newNavigationBar({
        title = language["mamatalkViewScene"]["title"],
        width = __appContentWidth__,
...
--    comment_box.anchorX = 0
    ...
    local default_comment_text = display.newText(group, language["mamatalkViewScene"]["input_comment"], 0, 0, __appContentWidth__ - 50, 30, native.systemFont, 12)
